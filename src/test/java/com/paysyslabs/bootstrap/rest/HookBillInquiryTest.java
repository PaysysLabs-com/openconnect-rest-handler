...
public class HookBillInquiryTest {
    ...
    public void request() throws Exception {
        ...
        second.setDataTemplate(
                ...
                "   </soapenv:Body>\r\n" + 
                "</soapenv:Envelope>");
        second.setExpectedFields("user_id,password,client_id,channel_id,stan,ucid,consumer_no,signature,tran_ref");
        second.setRequestFormat(WSRequestFormat.SOAP);
        second.setRequestHeaders("Content-Type: text/xml || SOAPAction: \"http://tempuri.org/BillInquiry\"");
        second.setResponseFormat(WSResponseFormat.XML);
        second.setResponseCodePath(null);
        second.setResponseIncludePaths("s|Envelope>s|Body>BillInquiryResponse>BillInquiryResult");
        second.setGuaranteed(false);

        second = endpointConfigRepository.save(second);
        Map<String, String> request = new HashMap<>();
        request.put("realm", "1link-ubps");
        request.put("type", "bill-inquiry");
        request.put("stan", "253456");
        request.put("ucid", "TELNOR01");
        request.put("consumer_no", "03452653695");
        request.put("tran_ref", "123456789012");
